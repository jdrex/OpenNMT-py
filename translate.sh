...
srun --gres=gpu:1 --time=240:00:00 --partition=sm --exclude sls-sm-5 python translate.py -data_type audio -model $1 -src_dir data/speech -src scp:data/wsj/src-test-feats.scp \
    -output test.pred.$2.$1 -tgt data/wsj/tgt-test.txt.chars -gpu 0 -verbose -max_length 200 -min_length 15 -beam_size $2 > log.test.pred.$2.$1 2>&1 &
