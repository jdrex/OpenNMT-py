...
srun --gres=gpu:1 --time=240:00:00 --partition=sm --exclude=sls-sm-3 python translate.py -data_type audio -model $1 -src_dir data/speech -src scp:data/wsj/src-val-feats.scp \
    -output val.pred.$2.$1 -tgt data/wsj/tgt-val.txt.chars -gpu 0 -verbose -max_length 200 -min_length 15 -beam_size $2 -report_attn -1 "${@:3}" -batch_size 1 > log.val.pred.$2.$1 2>&1 &
