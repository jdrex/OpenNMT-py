...
def train_model(auto_models, valid_model, train_data, valid_data, fields_list, valid_fields, optims,
    ...
    src_trainer = onmt.Trainer(auto_models[0], src_train_iter, valid_iter,
                               src_train_loss, valid_loss, optims[0], trunc_size, shard_size)
    ...
    tgt_trainer = onmt.Trainer(auto_models[1], tgt_train_iter, valid_iter,
                               tgt_train_loss, valid_loss, optims[1], trunc_size, shard_size)

    src_train_iter = make_train_data_iter(train_data[0], opt)
    tgt_train_iter = make_train_data_iter(train_data[1], opt)
    src_train_loss = make_loss_compute(auto_models[0], fields_list[0]["tgt"].vocab, train_data[0], opt)
    tgt_train_loss = make_loss_compute(auto_models[1], fields_list[1]["tgt"].vocab, train_data[1], opt)
    unsup_trainer = onmt.UnsupTrainer(auto_models, [None,None], discrim_models, [src_train_iter, tgt_train_iter], valid_iter,
                                     [src_train_loss, tgt_train_loss], [None, None], valid_loss, [optims[0], None], [None, None],
...
def main():
    ...
    valid = torch.load(opt.data + '.' + src_lang + '-' + tgt_lang + '.valid.pt')
    ...
    fields_valid = load_fields(valid, checkpoint, lang=src_lang + '-' + tgt_lang)
    ...
    labels = [0.1, 0.9]
    train_model(models, valid_model, train, valid, fields, fields_valid, optims,
                discrim_models, discrim_optims, labels, [advers_optims[0], None])
